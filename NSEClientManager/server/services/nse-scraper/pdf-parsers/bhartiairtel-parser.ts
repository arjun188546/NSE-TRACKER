/**
 * Bharti Airtel PDF Parser
 * Handles Bharti Airtel's quarterly result PDF format
 */

import { CompanyPDFParser, FinancialMetrics } from './base-parser';
import { nseClient } from '../http-client';

export class BhartiAirtelParser extends CompanyPDFParser {
  constructor() {
    super('BHARTIARTL', 'Bharti Airtel Limited');
  }
  ...
  protected async extractMetrics(text: string): Promise<FinancialMetrics> {
    console.log(`[BHARTIARTL Parser] Extracting metrics from ${text.length} characters`);
    ...
    metrics.revenue = this.extractNumber(text, [
      ...
      /Mobile\s+services\s+revenue[\s\S]{0,200}?([\d,]+\.?\d*)/i,
    ...
    console.log(`[BHARTIARTL Parser] Found Revenue: ${metrics.revenue || 'N/A'}`);
    console.log(`[BHARTIARTL Parser] Found Net Profit: ${metrics.netProfit || 'N/A'}`);
    console.log(`[BHARTIARTL Parser] Found EPS: ${metrics.eps || 'N/A'}`);
    console.log(`[BHARTIARTL Parser] Found EBITDA: ${metrics.ebitda || 'N/A'}`);
