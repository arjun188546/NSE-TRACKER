/**
 * Tata Steel PDF Parser
 * Handles Tata Steel's quarterly result PDF format
 */

import { CompanyPDFParser, FinancialMetrics } from './base-parser';
import { nseClient } from '../http-client';

export class TataSteelParser extends CompanyPDFParser {
  constructor() {
    super('TATASTEEL', 'Tata Steel Limited');
  }
  ...
  protected async extractMetrics(text: string): Promise<FinancialMetrics> {
    console.log(`[TATASTEEL Parser] Extracting metrics from ${text.length} characters`);
    ...
    metrics.ebitda = this.extractNumber(text, [
      ...
      /Operating\s+(?:profit|income)[\s\S]{0,200}?([\d,]+\.?\d*)/i,
    ...
    console.log(`[TATASTEEL Parser] Found Revenue: ${metrics.revenue || 'N/A'}`);
    console.log(`[TATASTEEL Parser] Found Net Profit: ${metrics.netProfit || 'N/A'}`);
    console.log(`[TATASTEEL Parser] Found EPS: ${metrics.eps || 'N/A'}`);
