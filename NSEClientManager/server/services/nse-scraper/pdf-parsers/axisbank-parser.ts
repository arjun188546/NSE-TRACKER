 ...
 * Handles Axis Bank's quarterly result PDF format
 */

import { CompanyPDFParser, FinancialMetrics } from './base-parser';
import { nseClient } from '../http-client';

export class AxisBankParser extends CompanyPDFParser {
  constructor() {
    super('AXISBANK', 'Axis Bank Limited');
  }
  ...
  protected async extractMetrics(text: string): Promise<FinancialMetrics> {
    console.log(`[AXISBANK Parser] Extracting metrics from ${text.length} characters`);
    ...
    console.log(`[AXISBANK Parser] Found Revenue: ${metrics.revenue || 'N/A'}`);
    console.log(`[AXISBANK Parser] Found Net Profit: ${metrics.netProfit || 'N/A'}`);
    console.log(`[AXISBANK Parser] Found EPS: ${metrics.eps || 'N/A'}`);
